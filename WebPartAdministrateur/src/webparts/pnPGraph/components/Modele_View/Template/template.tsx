...
import {IUserItemProp} from '../../Models/IUserItemProp';
import {TempState} from '../../Models/TempState';
import pnp, { ItemAddResult, Item} from "sp-pnp-js";
import jsPDF from 'jsPDF';
import html2canvas from 'html2canvas';
import { Toggle } from 'office-ui-fabric-react/lib/Toggle';
export class Template extends React.Component<IUserItemProp,TempState> {
...
`
		...
		pnp.sp.web.lists.getByTitle("FavoriUsers").items.select('Title,Id').get().then(

			r => {
			  for (var i = 0; i < r.length; i++) {
				  
			if(this.props.userPrincipalName==r[i].Title){
			this.setState({favoriImage:false});
			console.log("existe")
		}
    ...
    for (let i = 0; i <this.props.pastProjects.length; i++) {
		let	verif=true;
			this.props.projects.value.forEach(element => {
				if(element.fields.Title==this.props.pastProjects[i])
				{
				
					verif=false;
					table+=(`
					<li><strong>`+this.props.pastProjects[i]+" : " +element.fields.Description+`</strong> </li>`)
				}
	...
	{
		return '/_layouts/15/userphoto.aspx?size=L&username='+this.props.userPrincipalName
	}

	private addFavori(item:string){
		pnp.sp.web.lists.getByTitle("FavoriUsers").items.add({
		  Title: item
    ...
    private deleteFavori(){
		let list = pnp.sp.web.lists.getByTitle("FavoriUsers");
		pnp.sp.web.lists.getByTitle("FavoriUsers").items.select('Title,Id').get().then(
			r => {
			  // look through the returned items.
			  for (var i = 0; i < r.length; i++) {
					if(this.props.userPrincipalName==r[i].Title)
					list.items.getById(r[i].Id).delete().then(_ => {
						alert("item deleted")
					  });
	...
	<span dangerouslySetInnerHTML={{__html:finalstr}} />
	</div>
	...
	private printDocument() {
		...
		html2canvas(input)
			.then((canvas) => {
				...
				pdf.save(this.props.displayName+".pdf");
			})
		;
	}
	private onclick(){
		...
		this.addFavori(this.props.userPrincipalName);
		}
		else
		{this.setState({favoriImage:true});
		this.deleteFavori();		
	}} 	
private favori=function(){
...
<Toggle
    ...
    onClick={this.onclick.bind(this)}
        ...
        />
				<a target="_blank" href={"https://cvgenerator-admin.sharepoint.com/_layouts/15/tenantprofileadmin/ProfMngr.aspx?ConsoleView=Active&ProfileType=User&ApplicationID=00000000-0000-0000-0000-000000000000"} >  Modifier ce profil </a>
</div>
);
}	
